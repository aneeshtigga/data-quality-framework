            ...
            for table_name in table_list:
                ...
                try:
                    ...
                    for cols in table_meta['columns']:
                        StructFields.append(f"StructField(\"{cols['name']}\", {cols['type']}(), {cols['nullable']})")
                    Fields = ('[%s]' % ', '.join(map(str, StructFields)))
                    print(Fields)
                    schema = StructType([Fields])
                except Exception as e:
                    log.error(e)
                    log.error("Failed checking Schema file for table {}".format(table_name))
